      ...
      <div className="grid grid-cols-8 p-3 shrink-0 w-full gap-0.5">
        {query.isSuccess &&
          query.data.map(album => (
            <Card key={album.name} isFooterBlurred radius="none" className="aspect-square group">
              <Cover className="size-full rounded-none" url={album.cover} placeholder={Disc3Icon} />

              <CardFooter
                className="absolute bg-background/60 bottom-0 z-10 py-2 px-3
                max-h-10 group-hover:max-h-full transition-[max-height]">
                <div className="text-small line-clamp-1 group-hover:line-clamp-none">{album.name}</div>
              </CardFooter>
            </Card>
          ))}
      </div>
